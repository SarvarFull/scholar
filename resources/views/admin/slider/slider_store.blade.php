            ...
            <ul class="list-unstyled ps-0">
                <div class="col-9 p-3">
                    <form action="{{ route('slider.add') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3">
                            <input type="file" name="image" class="form-control @error('image') in-valid @enderror"
                                value="{{ old('image') }}">
                            @error('image')
                        ...
                        <div class="mb-3">
                            <input type="text" name="title" placeholder="Title ..."
                                class="form-control @error('title') in-valid @enderror" value="{{ old('title') }}">
                            @error('title')
                                <div class="alert alert-danger alert-dismissible fade show mt-1" role="alert">
                                    {{ $message }}
                                    <button type="button" class="btn-close" data-bs-dismiss="alert"
                        ...
                        <div class="mb-3">
                            <input type="text" name="content" placeholder="Content ..."
                                class="form-control @error('content') in-valid @enderror" value="{{ old('content') }}">
                            @error('content')
                                <div class="alert alert-danger alert-dismissible fade show mt-1" role="alert">
                                    {{ $message }}
                                    <button type="button" class="btn-close" data-bs-dismiss="alert"
